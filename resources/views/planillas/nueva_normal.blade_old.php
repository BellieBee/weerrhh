		  	...
		  	<div id="planilla" class="tab-pane fade in active">
	            ...
	            @foreach($users as $user)    			    
			        ...
			        <tr class="aportes{{$user->id}}">				        
				         @if($pais->id == 7 || $pais->id==1 || (str_contains($fecha, 'Junio') && $pais->id==4 ) )
							...
							@if($pais->id == 7)
								<td>
									...
									<input type="number" step="0.01" class="form-control" name="planilla[{{$user->id}}][auxilio_transporte]" 
									@if($edit) 
			        ...
			        <tr class="patronales">
						...
						@if($pais->id == 6)
							@if($edit)
								@if($planilla)
									@if($planilla->created_at->format('Y-m-d') >= date('Y-m-d'))
										<td>
						...
						@if($pais->id == 7)
							<td>
				           		<label
						           	data-toggle="tooltip" 
					           		title="Salario * 1%">
					           		Interes 1%<i class="fa fa-question-circle" ></i>
					           	</label>
...
//////////////////////////////////////////////////////////////////////////////////
	...
	$(document).on('click ', '.calculo_acumulados', function(event) {
		...
		if(pais == 7) {
			...
			total_salario = (parseFloat(salario_base)/30) * parseFloat(dias_trabajados) + parseFloat(ajuste) + parseFloat(total_aportes);
			...
			parafiscales = (total_salario) * (9.00/100);
