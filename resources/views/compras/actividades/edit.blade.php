...
@include('partials.messages')
	...
	<div class="row">
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-6">
				<textarea class="form-control" required name="actividad">
					{{$actividad->actividad}}
				</textarea>
			</div>
		</div>
		...
		<div class="form-group" id="colegas_proveedores" {{$actividad->proveedor_user_id == null ? 'hidden' : ''}}>
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-4">
		      	<select class="form-control" name="tipo_moneda" 
		      		id="tipo_moneda">
					@if($actividad->tipo_moneda_id != null || $actividad->tipo_moneda_id != 0)
				      		...
				      		<tr>	      		
					      		<td>
						      		<select class="form-control" name="proyecto[]">
			      						...
			      						@foreach($proyectos as $proyecto)
										  	@if ($proyecto->id != $detalle->proyecto_id)
												<option value="{{$proyecto->id}}">[{{$proyecto->nombre}}] {{$proyecto->descripcion}}</option>	  
											@endif
			      						@endforeach
			      					</select>
								...
								<td>
					      		...
					      		<td>
					      			...
					      			<input type="hidden" name="oldF[]" value="{{$detalle->file}}">
					      		</td>
		...
		<div class="form-group monto_group">
		    ...
		    <div class="col-sm-2">
		      	<input type="text" class="form-control" id="n_factura" name="n_factura" value="{{$actividad->n_factura}}">
		    </div>--}}
		</div>
		...
		<div class="col-md-12 text-right">
			...
			@if(Entrust::can(['editar-actividad']))
				<button type="submit" class="btn btn_color submit" >
		           Actualizar Solicitud de Actividad
		        </button>
			@endif
	        ...
	        {{--@if(Entrust::can('aprobar-actividad-1') && $actividad->aprobacion_1 == 0 || (Entrust::can('aprobar-actividad-1-inferior') && auth()->user()->id != $actividad->user_id) && $actividad->aprobacion_1 == 0)
		        <a href='{{url("actividad/$actividad->id/aprobacion?aprobacion_1=1")}}' class="btn btn_color aprobar">
					<i class="fa fa-check-square"></i> 1ra Aprobación
				</a> 
				<a href='{{url("actividad/$actividad->id/aprobacion?aprobacion_1=2")}}' class="btn btn_color btn_rojo aprobar">
					 Rechazar
				</a> 
	        @endif
	        ...
	        @if(Entrust::can('aprobar-actividad-2') && $actividad->aprobacion_1 == 1 && $actividad->aprobacion_2 == 0 || (Entrust::can('aprobar-actividad-2-inferior') && auth()->user()->id != $actividad->user_id) && $actividad->aprobacion_1 == 1 && $actividad->aprobacion_2 == 0)
	        	<a href='{{url("actividad/$actividad->id/aprobacion?aprobacion_2=1")}}' class="btn btn_color aprobar">
					<i class="fa fa-check-square"></i> 2ra Aprobación
				</a> 
				<a href='{{url("actividad/$actividad->id/aprobacion?aprobacion_2=2")}}' class="btn btn_color btn_rojo aprobar">
					Rechazar
				</a> 
	        @endif
