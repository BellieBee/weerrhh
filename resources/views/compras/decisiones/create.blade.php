...
<div class="row">
    ...
    <div class="col-lg-12">
    	<h4>Solicitud Actividad {{$actividad->correlativo}}</h4><br>
    </div>
</div>

@include('partials.messages')

	<form class="form-horizontal bonos_form" action="{{route('decision.store')}}" method="post" enctype="multipart/form-data">
		{!! csrf_field() !!}
		<input type="hidden" name="correlativo" value="{{$correlativo}}">
		<input type="hidden" name="actividad_id" value="{{$actividad->id}}">
		<input type="hidden" name="oficina_id" value="{{$actividad->oficina_id}}">
		@if($actividad->proveedor_id != 0 || $actividad->proveedor_id != null)
			<input type="hidden" name="proveedor_id" value="{{$actividad->proveedor_id}}">
		@elseif ($actividad->proveedor_user_id != 0 || $actividad->proveedor_user_id != null)
	...
	<div class="row">
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-2">
		      	<input type="text" class="form-control" disabled  value="{{$correlativo}} " >
		    </div>
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-4">
		      	<input type="text" class="form-control" disabled  value="{{$actividad->oficina->oficina}}">
		    </div>
		</div>
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-6">
				<textarea class="form-control" required name="antecedentes"></textarea>
			</div>
		</div>
				      	...
				      	@foreach($actividad->documento as $key => $documento)
				      		<tr>
					      		...
					      		<td> 
				      				...
				      				<a href='{{url("actividad/$documento->id/deleteDoc")}}'  class="btn btn_color btn_rojo" 
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-6">
				<textarea class="form-control" required name="decision"></textarea>
			</div>
		</div>
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-4">
		    	@if($actividad->proveedor_id != 0 || $actividad->proveedor_id != null)
		    		<input type="text" class="form-control" disabled  value="{{$actividad->proveedor->razon_social}}">
				@elseif ($actividad->proveedor_user_id != 0 || $actividad->proveedor_user_id != null)
					<input type="text" class="form-control" disabled  value="{{$actividad->proveedorUser->first_name}} {{$actividad->proveedorUser->last_name}}">
		    	@else
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-6">
				<textarea class="form-control" required name="notas"></textarea>
			</div>
		</div>
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-6">
				<textarea class="form-control" required name="justificacion"></textarea>
			</div>
		</div>

		@if($actividad->monto = "0.00")
			<div class="form-group">
			...
			<div class="form-group monto_group">
				...
				<div class="col-sm-2">
				    {{$actividad->tipo_moneda_id != 0 ? $actividad->tipo_moneda->moneda_simbolo : ''}} <input type="number" step="0.0000001" class="form-control monto_total" name="monto_total" id="monto_total" value="{{$actividad->monto}}"> {{$actividad->tipo_moneda_id != 0 ? $actividad->tipo_moneda->moneda_nombre : ''}}
				</div>
				<label class="control-label col-sm-2">Tipo Moneda:</label>		    
				<div class="col-sm-2">
				    ($) <input type="number" step="0.0000001" class="form-control tipo_cambio_usd" name="tipo_cambio_usd" id="tipo_cambio_usd" value="{{$actividad->oficina->pais->tasa_conv_usd}}"> (SK) <input type="number" step="0.0000001" class="form-control tipo_cambio_corona" name="tipo_cambio_corona" id="tipo_cambio_corona" value="{{$actividad->oficina->pais->tasa_conv_corona}}"> 
				</div>		
			</div>
		...
		<div class="col-md-12 text-right">
			...
			@if(Entrust::can(['crear-decision-todos','crear-decision-oficina']))
...
<script type="text/javascript">	
	...
	$(document).on('change', '.monto_total, .tipo_cambio_usd, .tipo_cambio_corona', function (event) {
		...
		tipo_cambio_usd = $("#tipo_cambio_usd").val();
		tipo_cambio_corona = $("#tipo_cambio_corona").val();
		//monto_usd = $("#monto_usd").val();
		//monto_sk = $("#monto_sk").val();
