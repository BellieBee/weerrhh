<table>
    <tr>
        ...
        @if($pais->id==1 || $pais->id == 7)<!--//GUATEMALA-->					
    ...
    <tr class="din" style="font-weight: bold;">
        ...
        <!-- total deducciones-->			
            ...
            @elseif($pais->id == 7)<!--//COLOMBIA-->				
                <td>{{number_format($empleado->deduccion->sum('salud'),2)}}</td>	
                <td>{{number_format($empleado->deduccion->sum('fsp'),2)}}</td>	
                <td>{{number_format($empleado->deduccion->sum('pen'),2)}}</td>	
                <td>{{number_format($empleado->deduccion->sum('rte'),2)}}</td>
            @endif
        ...
        <!--aportes-->	
            ...
            @if($pais->id == 7)<!--//COLOMBIA-->
                <td>{{number_format($empleado->aporte->sum('auxilio_transporte'),2)}}</td>
                <td>{{number_format($empleado->aporte->sum('bono_salud'),2)}}</td>
                <td>{{number_format($empleado->aporte->sum('capacitacion'),2)}}</td>
                <td>{{number_format($empleado->aporte->sum('seguro_medico'),2)}}</td>
                <td>{{number_format($empleado->aporte->sum('total_aportes'),2)}}</td>
            @endif
