...
@include('partials.messages')
	...
	<form class="form-horizontal bonos_form" action="{{ route('bonosalud.update', $bono_salud->id)}}" method="post" enctype="multipart/form-data">
		{!! csrf_field() !!}
		<input type="hidden" name="user_id" value="{{$user->id}}">
		<input type="hidden" name="oficina_id" value="{{$user->oficina_id}}">
		<input type="hidden" name="id" value="{{$bono_salud->id}}">
	...
	<div class="row">
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-2">
		      	<input type="text" name="saldo_inicial" class="form-control" disabled value="{{$bono_salud->saldo_inicial}}" id="saldo_inicial" step="0.0000001" disabled>
		    </div>
		    <label class="control-label col-sm-2">Costo Moneda Local:</label>
		    <div class="col-sm-2">
		      	<input name="costo_moneda_local" type="number" min="0" class="form-control" value="{{$bono_salud->costo_moneda_local}}" id="costo_moneda_local" step="0.0000001">
		    </div>
		</div>	
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-2">
		      	<input type="number" name="tipo_cambio_sk" class="form-control" value="{{$bono_salud->tipo_cambio_sk}}" id="tipo_cambio_sk" step="0.0000001">
		    </div>
		    <label class="control-label col-sm-2" >Tipo Cambio USD:</label>		    
		    <div class="col-sm-2">
		      	<input type="number" name="tipo_cambio_usd" class="form-control" value="{{$bono_salud->tipo_cambio_usd}}" id="tipo_cambio_usd" step="0.0000001">
		    </div>			
		</div>
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-2">
		      	<input name="costo_sk" type="number" min="0" class="form-control" disabled id="costo_sk"
		      	value="{{$bono_salud->costo_sk}}" step="0.0000001" disabled>
		    </div>
		    <label class="control-label col-sm-2" >Saldo Final (sk):</label>
		    <div class="col-sm-2">
		      	<input name="saldo_final" type="number" min="0" class="form-control" disabled id="saldo_final"
		      	value="{{$bono_salud->saldo_final}}" step="0.0000001" disabled>
		    </div>
		</div>
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-4">
		      	<input type="text" name="destino" class="form-control" value="{{$bono_salud->destino}}">
		    </div>
		</div>		
			...
			<div class="col-sm-6 ">
				...
				<table class=" table table-bordered">
				    ...
				    <tbody class="clone_documentos">
				      	...
				      	@foreach($bono_salud->documento as $key => $documento)
				      		<tr class="documentos_input_file" > 				
				      			<td class="num_docs" align="center">{{$key+1}}</td>

				      			<td>
				      				<input name="nombre_documento[]"  class="form-control nombre_documento"  type="text" value="{{$documento->nombre_documento}}">
				      			</td>
				      			...
				      			<td>
				      				<a href='{{route('bonosalud.downloadDocument', $documento->documento)}}'
				      					 
				      					 class="btn btn_color btn_download"
				      					 style="background: #5cb85c;">
				      						<i class="glyphicon glyphicon-download-alt"></i>
				      				</a> 
				      				<a href='{{url("bonosalud/$documento->id/deleteDoc")}}'  class="btn btn_color btn_rojo" 
		...
		<div class="col-md-12 text-right">
			...
			@if(Entrust::can(['crear-bonossalud-todos','crear-bonossalud-oficina','crear-bonossalud-solo']))
				<button type="submit" class="btn btn_color submit" >
		           Actualizar Solicitud de Bono Salud
		        </button>
			@endif

	        @if(Entrust::can('aprobar-bonossalud') || (Entrust::can('aprobar-bonossalud-inferior') && auth()->user()->id != $bono_salud->user_id) && $bono_salud->status == 0)
	        <a href='{{url("bonosalud/$bono_salud->id/aprobacion?status=1")}}' class="btn btn_color aprobar">
				<i class="fa fa-check-square"></i> Aprobar Bono Salud
			</a> 
			<a href='{{url("bonosalud/$bono_salud->id/aprobacion?status=2")}}' class="btn btn_color btn_rojo aprobar">
				 Rechazar
			</a> 
	        @endif

	        @if($bono_salud->status == 1 && Entrust::can('anular-bonossalud') || (Entrust::can('anular-bonossalud-inferior') && auth()->user()->id != $bono_salud->user_id))
	        	<a href='{{url("bonosalud/$bono_salud->id/aprobacion?status=3")}}' class="btn btn_color btn_rojo aprobar">
					Anular Bono Salud
				</a>
			@endif
    	</div>
